    ...
    def find_matches_in(self, content):
        ...
        for word, index in words_to_compare:
            ...
            if not self.case_sensitive:
                ...
                #score = fuzz.ratio(self.query.lower(), word.lower()[:min(self.qlen, len(word))])
            else:
                score = get_fuzzy_score(self.query, self.qlen, word)
                #score = fuzz.ratio(self.query, word[:min(self.qlen, len(word))])
...
def get_fuzzy_score(query, qlen, word):
    return fuzz.ratio(query, word[:qlen])